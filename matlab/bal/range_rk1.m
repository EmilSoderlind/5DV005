...
%
% All time steps have the same size, except the final step which is adjusted 
% to put the shell exactly on the ground.
%
...
%
% See also: COMPUTE_RANGE, COMPUTE_ELEVATION, FIRE, RANGE_RKX, TARGET
...
% Define the initial condition; must be compatible with the shell model
...
for it=1:maxstep
    ...
    % Advance the solution a single step using Euler's method
    tra(:,it+1)=phi1(shell,t(it),tra(:,it),dt);

    % Test to see if we are below ground level, i.e. tra(2,it+1)<0
    if (tra(2,it+1)<0)
        %------------------------------------------------------------------
        % We passed through the ground! Go back to the previous instant
        % and compute the 'exact' time step which will put the shell on the
        % ground! This is simpler than it seems. Always make a sketch!
        %------------------------------------------------------------------
        
        % The height of the shell is tra(2,it) and the y-velocity of is 
        % aux(2), where
